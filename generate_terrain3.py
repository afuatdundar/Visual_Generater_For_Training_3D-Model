...
def export_terrain(file_number):
    ...
    pyautogui.click(x=82, y=501)  # Export terrain butonunun koordinatları
    time.sleep(2)
...
for seed in range(start_seed, end_seed + step, step):
    ...
    pyautogui.click(x=94, y=201)  # İlk Billowy noise elemanının koordinatları
    time.sleep(2)
    set_seed_and_generate(seed, 1650, 1009)
    
    
    # İkinci Billowy noise elemanı için
    pyautogui.click(x=89, y=219)  # İkinci Billowy noise elemanının koordinatları
    time.sleep(2)
    set_seed_and_generate(seed, 1650, 1009)
