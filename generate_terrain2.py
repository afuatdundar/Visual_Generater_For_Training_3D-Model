...
def set_seed_and_generate(seed, x, y):
    ...
    time.sleep(1)  # Değişikliklerin uygulanması için bekle

def export_terrain(file_number):
    # Export terrain butonuna git ve tıkla
    pyautogui.click(x=93, y=382)  # Export terrain butonunun koordinatları
    time.sleep(1)
    ...
    time.sleep(1)  # Export işlemi için bekle
...
for seed in range(start_seed, end_seed + step, step):
    ...
    pyautogui.click(x=89, y=244)  # İlk Billowy noise elemanının koordinatları
    time.sleep(1)
    set_seed_and_generate(seed, 1650, 1009)
    
    
    # İkinci Billowy noise elemanı için
    pyautogui.click(x=89, y=262)  # İkinci Billowy noise elemanının koordinatları
    time.sleep(1)
    set_seed_and_generate(seed, 1650, 1009)
